...
//pagamento de vale-transporte e nem o desconto.
 ...
 escreva ("netsalaru" + netSalary1)

   se (netSalary1 <= 2259.20){
                aliquot = 0
                deduction = 0
                irrf = (netSalary1 * aliquot) - deduction
            }
            senao se (netSalary1 <= 2826.65)
            {
                aliquot = 0.075
                deduction = 169.44
                irrf = (netSalary1 * aliquot) - deduction
            }
            senao se (netSalary1 <= 3751.05)
            {
                aliquot = 0.15
                deduction = 381.44
                irrf = (netSalary1 * aliquot) - deduction
            }
            senao se (netSalary1 <= 4664.68)
            {
                aliquot = 0.225
                deduction = 662.77
                irrf = (netSalary1 * aliquot) - deduction
 ...
 se (trasnportationVoucher > 0){
              netSalary = netSalary + transport - trasnportationVoucher
         }
           senao {
  }
  ...
  escreva ("Valor do transporte: R$" + transport + "\n")
  escreva ("Desconto do transporte: R$" + trasnportationVoucher + "\n")
